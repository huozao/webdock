...
WebDock 用真实浏览器自动化驱动 ChatGPT 网页，跑在 webdock2（主，Win11+WSL2）/ webdock1（备，Ubuntu 笔记本）。工作区级规则（设备、提交授权、排障路由）见上层 `../AGENTS.md`。
...
- 部署换镜像时 `.env` 和 systemd unit 两处 `WEBDOCK_IMAGE` 都要改。
