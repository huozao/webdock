...
class MediaStore:
    ...
    def put(self, data: bytes, content_type: str = "image/png") -> str:
        ...
        with self._lock:
            self._evict(now)
            self._store[token] = (data, content_type, now + self._ttl)
        return token
    ...
    def get(self, token: str) -> tuple[bytes, str] | None:
        ...
        with self._lock:
            ...
            if item is None:
                return None
            data, content_type, expire_at = item
            if expire_at < now:
    ...
    def _evict(self, now: float) -> None:
        expired = [token for token, (_, _, exp) in self._store.items() if exp < now]
        for token in expired:
            self._store.pop(token, None)
        while len(self._store) >= self._max:
            oldest = min(self._store, key=lambda t: self._store[t][2])
